...
class GemmaDinoImageCaptioner(nn.Module):
    ...
    def forward(self, images, captions):
        """
        :param images: list[PIL.Image] or Tensors that are accepeted by AutoImageProcessor
        ...
        self.vit.to(device)
        self.gemma.to(device)

        #prepare images for vit
        images = self.vit_processor(images, return_tensors="pt")["pixel_values"].to(device)
    ...
    def inference_generate(self, images, max_new_tokens=30, temperature = 0.0):
        ...
        :param images: list[PIL.image] or tensor accepted by AutoImageProcessor
